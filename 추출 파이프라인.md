...
## 4. 정적 추출 vs 동적(LLM 보조) 추출
저자의 철학에 따라, 데이터 추출 단계에서의 LLM 개입은 **최소화**되어야 합니다. 모듈의 성격에 따라 추출 방식을 두 가지로 엄격히 구분합니다.

*   **정적 검사 (Static Extraction):** 질문이 고정되어 있으므로 **LLM을 아예 사용하지 않습니다.** 종이나 텍스트 에디터에 스스로 답을 적어 `raw_store`에 저장합니다.
    *   *해당 모듈:* 생성 은유와 문장 완성, 맥락 속 가치 할당, 두려운 자기와 조기 경보, 판단 스타일 시나리오, 자기-타자, CIT, CCRT, 대표 인생 장면, 미니 ESM.
*   **동적 검사 (Facilitated Extraction):** 사용자의 대답에 맞춰 꼬리질문("왜 그것이 중요하죠?", "두 개념의 차이가 뭐죠?")을 이어가야 하므로 **LLM(챗봇)의 보조가 필수적**입니다.
    *   *해당 모듈:* 삼항 도출(Triadic), 래더링(Laddering).
