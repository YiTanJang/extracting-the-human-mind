...
각 에이전트에 주입되는 raw construct는 *그 단어를 말하라는 대본*이 아니라, *상황을 어떻게 해석할지의 렌즈*로 작동시킨다. 예: '자율성'이 construct라면, 에이전트가 "자율성"을 입에 담는 게 아니라, 통제권이 위협받는 입력을 *위협으로 지각*하도록 만든다. 나아가 한 명의 캐릭터(에이전트) 안에서도 단일 프롬프트에 의존하지 않고 **[인지 엔진](<인지 엔진.md>)**를 병렬로 가동하여, 이성적 규범(가치)과 방어적 본능(두려움) 간의 내적 갈등을 통과한 뒤에야 최종 발화가 결정되도록 입체성을 높인다.
