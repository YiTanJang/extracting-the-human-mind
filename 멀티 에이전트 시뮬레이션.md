...
각 에이전트는 **자기 원시 데이터만** 받는다. 한 사람의 데이터가 다른 에이전트의 발화에 새지 않게 한다.
...
- **충실도 미검증:** 라벨 없는 원시 데이터로 인스턴스화한 에이전트가 **실제 그 사람처럼** 반응하는지는 가정일 뿐이다. 이 가정의 검증이 곧 검증 모드의 존재 이유다([검증 모드](<검증 모드.md>)).
